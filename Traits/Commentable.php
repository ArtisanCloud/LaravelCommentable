...
trait Commentable
{
    ...
    {
        static::created(function (Model $commentableModel) {
            if (count($commentableModel->queuedTags) > 0) {
                $commentableModel->attachTags($commentableModel->queuedTags);

                $commentableModel->queuedTags = [];
            }
        });
    ...
    {
        return $this
            ->morphToMany(self::getTagClassName(), 'commentable');
//            ->ordered();
    }
